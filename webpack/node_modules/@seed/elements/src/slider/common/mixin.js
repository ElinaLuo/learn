/**
 * @since 2018-12-25 22:22
 * @author vivaxy
 */
import { prefixCls } from '@seed/ui-base';

const getHandlerXByValue = function(progressWidth, marks, value) {
  return (progressWidth / (marks.length - 1)) * value;
};

export const mixin = function(ins) {
  ins.classes = {
    slider: prefixCls('slider'),
    container: prefixCls('slider', 'container'),
    progress: prefixCls('slider', 'progress'),
    progressInactive: prefixCls('slider', 'progress-inactive'),
    marks: prefixCls('slider', 'marks'),
    markItem: prefixCls('slider', 'mark-item'),
    markPoint: prefixCls('slider', 'mark-point'),
    markText: prefixCls('slider', 'mark-text'),
    handler: prefixCls('slider', 'handler'),
    handlerBar: prefixCls('slider', 'handler-bar'),
  };

  // touch life circle
  ins.handlerStartClientX = null;
  ins.handlerStartX = null;

  // component life circle
  ins.progressEl = null;
  ins.handlerEl = null;
  ins.handlerWidth = null;
  ins.progressWidth = null;

  ins.getWidth = function() {
    const handlerEl = ins.isReactComponent
      ? ins.handlerEl
      : ins.$refs.handlerEl;
    const progressEl = ins.isReactComponent
      ? ins.progressEl
      : ins.$refs.progressEl;
    const { width: handlerWidth } = handlerEl.getBoundingClientRect();
    const { width: progressWidth } = progressEl.getBoundingClientRect();
    ins.handlerWidth = handlerWidth;
    ins.progressWidth = progressWidth;
  };

  ins.updateHandlerX = function(handlerX) {
    if (ins.isReactComponent) {
      ins.setState({ handlerX });
    } else {
      ins.handlerX = handlerX;
    }
  };

  ins.emitChange = function(value) {
    if (ins.isReactComponent) {
      ins.props.onChange(value);
    } else {
      ins.$emit('change', value);
    }
  };

  ins.whenMount = function() {
    ins.getWidth();
    window.addEventListener('resize', ins.whenResize);
    const marks = ins.isReactComponent ? ins.props.marks : ins.marks;
    const value = ins.isReactComponent ? ins.props.value : ins.value;
    const handlerX = getHandlerXByValue(ins.progressWidth, marks, value);
    ins.updateHandlerX(handlerX);
  };

  ins.whenUpdateProps = function(marks, value) {
    const handlerX = getHandlerXByValue(ins.progressWidth, marks, value);
    ins.updateHandlerX(handlerX);
  };

  ins.whenResize = function() {
    ins.getWidth();
    const marks = ins.isReactComponent ? ins.props.marks : ins.marks;
    const value = ins.isReactComponent ? ins.props.value : ins.value;
    ins.whenUpdateProps(marks, value);
  };

  ins.whenUnmount = function() {
    ins.handlerWidth = null;
    ins.progressWidth = null;
    window.removeEventListener('resize', ins.whenResize);
  };

  ins.handleTouchStart = function(e) {
    ins.handlerStartClientX = e.changedTouches[0].clientX;
    ins.handlerStartX = ins.isReactComponent
      ? ins.state.handlerX
      : ins.handlerX;
  };

  ins.handleTouchMove = (e) => {
    if (e.changedTouches.length !== 1) {
      return; // not standard touch move
    }

    let newHandlerX =
      e.changedTouches[0].clientX - ins.handlerStartClientX + ins.handlerStartX;
    if (newHandlerX < 0) {
      newHandlerX = 0;
    }
    if (newHandlerX > ins.progressWidth) {
      newHandlerX = ins.progressWidth;
    }

    ins.updateHandlerX(newHandlerX);
  };

  ins.getCurrentValue = function() {
    const marks = ins.isReactComponent ? ins.props.marks : ins.marks;
    const handlerX = ins.isReactComponent ? ins.state.handlerX : ins.handlerX;
    const eachMarkWidth = ins.progressWidth / (marks.length - 1);
    return Math.floor(handlerX / eachMarkWidth);
  };

  ins.getSiblingValue = function() {
    const marks = ins.isReactComponent ? ins.props.marks : ins.marks;
    const handlerX = ins.isReactComponent ? ins.state.handlerX : ins.handlerX;
    const eachMarkWidth = ins.progressWidth / (marks.length - 1);
    const currentValue = Math.floor(handlerX / eachMarkWidth);
    const remaining = handlerX % eachMarkWidth;
    if (remaining > eachMarkWidth / 2) {
      // to right
      return currentValue + 1;
    } else {
      // to left
    }
    return currentValue;
  };

  ins.handleTouchEnd = (e) => {
    if (e.changedTouches.length !== 1) {
      return;
    }

    ins.handlerStartClientX = null;
    ins.handlerStartX = null;

    const { progressWidth } = ins;
    const handlerX = ins.isReactComponent ? ins.state.handlerX : ins.handlerX;
    const marks = ins.isReactComponent ? ins.props.marks : ins.marks;

    const siblingValue = ins.getSiblingValue();
    const value = ins.isReactComponent ? ins.props.value : ins.value;
    if (siblingValue !== value) {
      ins.emitChange(siblingValue);
    } else {
      const newHandlerX = getHandlerXByValue(
        progressWidth,
        marks,
        siblingValue,
      );
      if (handlerX !== newHandlerX) {
        ins.updateHandlerX(newHandlerX);
      }
    }
  };
};
